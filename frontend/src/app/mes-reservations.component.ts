...
@Component({
  ...
  template: `
    <div class="mr-page">
      ...
      <div class="mr-wrap">
        ...
        <!-- Header -->
        <div class="mr-header">
          <button class="mr-back-btn" (click)="router.navigate(['/profile'])">
            <i class="pi pi-arrow-left"></i>
          </button>
          <div class="mr-header-text">
            <h1 class="mr-title">My bookings</h1>
            <p class="mr-subtitle">All your tickets and stays in one place</p>
          </div>
                ...
                <div class="mr-card mr-card-transport">
                  ...
                  <div class="mr-card-body">
                    ...
                    <div class="mr-card-actions">
                      ...
                      <button type="button" class="mr-action-btn mr-action-qr" (click)="downloadQr(res.transportReservationId)">
                        <i class="pi pi-qrcode"></i> QR
                      </button>
                      <button type="button" class="mr-action-btn mr-action-pdf" (click)="downloadPdf(res.transportReservationId, res.reservationRef)">
  ...
  styles: [`
    ...
    .mr-subtitle { font-size: 0.88rem; color: var(--text-muted); margin: 0.2rem 0 0; }
...
export class MesReservationsComponent implements OnInit {
  ...
  loadReservations() {
    ...
    forkJoin({
      transport: this.dataSource.getMyTransportReservations().pipe(
        catchError(() => of([] as TransportReservation[]))
      ),
      accommodation: this.dataSource.getMyAccommodationReservations().pipe(
  ...
  cancelTransport(res: TransportReservation) {
    ...
    this.dataSource
      .cancelTransportReservation(res.transportReservationId)
      .pipe(
        catchError(() => {
          this.error.set('Could not cancel this transport booking right now.');
          return EMPTY;
        }),
  ...
  cancelHebergement(res: AccommodationReservation) {
    ...
    this.dataSource
      .cancelAccommodationReservation(res.id)
      .pipe(
        catchError(() => {
          this.error.set('Could not cancel this accommodation booking right now.');
          return EMPTY;
        }),
  ...
  goModifyTransport(_res: TransportReservation) {
  ...
  downloadQr(reservationId: number): void {
    this.http.get(`/api/tickets/${reservationId}/qr`, { responseType: 'blob' }).subscribe({
      next: blob => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `qr-${reservationId}.png`;
        a.click();
        URL.revokeObjectURL(url);
      },
      error: () => this.error.set('Could not download the QR code.')
    });
  }

  downloadPdf(reservationId: number, ref: string): void {
    this.http.get(`/api/tickets/${reservationId}/pdf`, { responseType: 'blob' }).subscribe({
      next: blob => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
      ...
      error: () => this.error.set('Could not download the PDF ticket.')
    });
  }
